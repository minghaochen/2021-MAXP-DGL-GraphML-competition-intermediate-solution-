...
def generate_preds_path(args):
    path = os.path.join(args.probs_dir, args.dataset, 
                        ...
                        f"_K_{args.K}_label_K_{args.label_K}_probs_seed_*_stage_{args.stage}_fold_1.pt")
    return path

def evaluator(preds, labels):
    return torch.sum(preds == labels) / torch.tensor(labels.shape[0])

def calculate_metrics(probs_list, labels, train_nid, val_nid, test_nid, args):
    train_results = []
    val_results = []
    test_results = []
...
def main(args):
    ...
    data = load_dataset(device, args, fold=1)
    ...
    print("-"*10 + " Before " + "-"*10)
    calculate_metrics(probs_list, labels, train_nid, val_nid, test_nid, args)
    ...
    calculate_metrics(processed_preds_list, labels, train_nid, val_nid, test_nid, args)
...
def define_parser():
    ...
    parser.add_argument("--use-labels", action="store_true", 
                        help="Whether to enhance base model with a label model", default=False)
