...
class SAGN(nn.Module):
    def __init__(self, in_feats, hidden, out_feats, num_hops, n_layers, num_heads, weight_style="attention", alpha=0.5, focal="first",
        ...
        # 增加节点信息
        # self.post_encoder = GroupMLP(hidden+50+24+32+23, hidden+50+24+32+23, out_feats, num_heads, n_layers, dropout)
        self.post_encoder = GroupMLP(hidden + 50+24 + 32*3, hidden + 50+24+32*3, out_feats, num_heads,
                                     n_layers, dropout)
    ...
    def forward(self, feats, node_info):
        out = 0
        feats = [self.input_drop(feat) for feat in feats]
        if self.pos_emb is not None:
        ...
        # 增加节点信息
        node_info = node_info.unsqueeze(1).type(torch.float32)
        out = torch.cat([out,node_info], dim=2)
        ############
...
class SLEModel(nn.Module):
    ...
    def forward(self, feats, node_info, label_emb):
        out = 0
        if self.base_model is not None:
            out = self.base_model(feats, node_info)
